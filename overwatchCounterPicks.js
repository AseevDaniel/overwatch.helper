...
enemyTeam.forEach((hero) => {
  ...
  hero.counteredBy.forEach((counteredItem) => {
    ...
    newItem.textContent =
      heroes.find((hero) => hero.id === counteredItem)?.name ||
...
const counterHeroes = Object.keys(bestCounters).map((element) =>
  heroes.find((h) => h.id === element)
);
...
if (bestTeam.length < 5)
  bestTeam.forEach((hero) => {
    ...
    roleImg.src = roles[hero.role];
    el.textContent =
      hero.name + " Counter " + bestCounters[hero.id] + " heroes";
